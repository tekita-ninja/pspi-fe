...
imageFront.value = {
    preview:
        controller.detail?.image_front &&
        toAssetLink(controller.detail.image_front),
};
imageLeft.value = {
    preview:
        controller.detail?.image_left &&
        toAssetLink(controller.detail?.image_left),
};
imageBack.value = {
    preview:
        controller.detail?.image_back &&
        toAssetLink(controller.detail?.image_back),
};
imageRight.value = {
    preview:
        controller.detail?.image_right &&
        toAssetLink(controller.detail?.image_right),
};
...
const onSubmit = form.handleSubmit(async (values) => {
    ...
    if (imageFront.value?.file) {
        responseImageFront = (
            await controllerUpload.uploadFile(imageFront.value?.file)
        ).data;
    }
    if (imageBack.value?.file) {
        responseImageBack = (
            await controllerUpload.uploadFile(imageBack.value?.file)
        ).data;
    }
    if (imageLeft.value?.file) {
        responseImageLeft = (
            await controllerUpload.uploadFile(imageLeft.value?.file)
        ).data;
    }
    if (imageRight.value?.file) {
        responseImageRight = (
            await controllerUpload.uploadFile(imageRight.value?.file)
        ).data;
    }
    if (terraDoc.value?.file) {
        responseTerraDoc = (
            await controllerUpload.uploadFile(terraDoc.value?.file)
        ).data;
    }
    ...
    <ContainersPage title="Armada" subtitle="Form Armada">
        ...
        <div>
            <div class="border p-3 rounded">
                ...
                <div
                    class="grid grid-cols-2 md:grid-cols-4 my-4 gap-2 md:gap-4"
                >
                    <div class="relative">
                        <label
                            for="image_front"
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <img
                                class="w-full h-full object-contain"
                                v-if="imageRight?.preview"
                                :src="imageRight?.preview"
                                alt="preview"
                            />
                            <div
                                v-if="!imageRight?.preview"
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <img
                                class="w-full h-full object-contain"
                                v-if="imageLeft?.preview"
                                :src="imageLeft?.preview"
                                alt="preview"
                            />
                            <div
                                v-if="!imageLeft?.preview"
                ...
                <a
                    v-if="controller.detail?.terra_doc"
                    class="text-sm font-bold text-blue-500"
                    :href="toAssetLink(controller.detail.terra_doc)"
                    target="_blank"
            ...
            <div class="mt-6 border p-3 rounded">
                <form @submit="onSubmit">
                    <div class="space-y-3">
                        <div class="grid md:grid-cols-2 gap-3">
                            <UiFormField
                                ...
                                name="nopol"
                                v-bind:model-value="controller.detail?.nopol"
                            >
                                <UiFormItem>
                            ...
                            >
                                <UiFormItem>
                                    ...
                                    <UiFormControl>
                                        <UiSelect
                                            v-bind="componentField"
                                            :default-value="`${controller.detail?.defaultDriverId}`"
                                        >
                                            <UiSelectTrigger>
                                                <UiSelectValue
                                            ...
                                            <UiSelectContent>
                                                <UiSelectGroup>
                                                    <UiSelectLabel
                                                        >main
                                                        driver</UiSelectLabel
                                                    >
                                                    <UiSelectItem
                                                        v-for="item in controllerDriver.lists"
                                                        :key="item.id"
                                                        :value="`${item.id}`"
                                                    >
                                                        {{ item.name }}
                                    ...
                                    <UiFormMessage />
                                    <UiFormDescription>
                                        Main Driver Adalah supir utama pada
                                        armada, dapat diubah pada saat pembuatan
                                        pengiriman jika yg bersangkutan tidak
                                        hadir atau terjadi kendala.
                                    </UiFormDescription>
                                </UiFormItem>
                            </UiFormField>
                        </div>
                        <UiFormField
                            v-slot="{ componentField }"
                            name="merk"
                            :model-value="controller.detail?.merk"
                        >
                            <UiFormItem>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <UiFormField
                                v-slot="{ componentField }"
                                name="quantityOpt"
                                :model-value="controller.detail?.quantity"
                                    ...
                                    <UiFormControl>
                                        <UiSelect v-bind="componentField">
                                            ...
                                            <UiSelectContent>
                                                <UiSelectGroup>
                                                    <UiSelectLabel
                                                        >quantity</UiSelectLabel
                                                    >
                                                    <UiSelectItem
                                                        v-for="item in optionQuantityArmada"
                                                        :key="item.label"
                                                        :value="`${item.label}`"
                                                    >
                                                        {{ item.label }}
                            ...
                            <UiFormField
                                v-slot="{ componentField }"
                                name="kompartment"
                                :model-value="controller.detail?.kompartment"
                            >
                                <UiFormItem>
                                    <UiFormControl>
                                        <UiInput
                    ...
                    <UiDialogFooter class="mt-4 flex justify-end">
                        <UiButton
                            @click="router.push('/admin/armada')"
                            variant="outline"
                            type="button"
                        >
                            Cancel
